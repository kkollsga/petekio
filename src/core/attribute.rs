    ...
    pub fn validate(&self) -> Result<()> {
        if self.id.trim().is_empty() || self.label.trim().is_empty() {
            return Err(GeoError::Parse(
                "attribute metadata id and label must be non-empty after trimming".into(),
            ));
        }
        if self
            .units
        ...
        {
            return Err(GeoError::Parse(
                "attribute metadata units must be non-empty after trimming or null".into(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn rejects_whitespace_only_descriptor_strings() {
