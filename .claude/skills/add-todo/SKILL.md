...
The folder it writes into is mapped by **`dev-docs/README.md`** (the canonical
layout) — `todos.md` is the lean index, `plans/` holds the detail. The whole
point is to capture fast *and* scope well, so the item is actionable later
without rediscovery. Do the work directly; only ask the user when a
...
- **Batch / deeper analysis** — a body of findings (a research report, an
  audit, a code review, inbox content) that contains *several* actionable
  items. Decompose it first, then run the per-item logic for each. See
  **§0 Decompose** before the single-item steps.
...
- **Group by theme.** Items that share a subsystem/layer (e.g. several `io/`
  reader findings, several `analysis/` ops) go in *one* `plans/` doc as
...
- **Enhancement / optimization / code-health / refactor / new reader or op** →
  `## Engineering backlog (live)`
- **Migration / follow-up from a shipped phase** → `## Migration follow-ups`
...
For anything touching code, spend one or two `grep`/`Read` calls (or the
code-review MCP once code exists) to **pin the fix site** (file:line, and which
layer — `foundation/io/core/analysis/manager/py`) and confirm it's real — a
scoped entry with a concrete location is worth far more than a vague one. For a
claimed bug, confirm it's a real defect and not intended behaviour (read the
surrounding code/tests, and check it against root `SPEC.md`/`API.md` — the
locked contract may make the "bug" deliberate). Convert any relative date to an
absolute one.

## 4. Choose the detail home (reuse first)
- **Fits an existing `plans/` doc's theme** (e.g. another `io/` reader, another
  code-health item) → append a section to that doc. Prefer this.
...
- **Layer + fix site + approach** — `file:line` (or target module under the
  layered tree) + the shape of the change; note any `API.md` signature it must
  match (the contract is locked — implement toward it, don't drift).
- **Evidence** — reproduction / bench / a failing golden test, if any.
- **Test** — for correctness work, the golden/unit test the fix must land
  (round-trip, hand-calc, analytic check — see CLAUDE.md "Code health").
- **Effort** — rough size.

## 5. Add the lean backlink
Append **one line** to the chosen `todos.md` section:
`- <short title> → [plans/<doc>.md](plans/<doc>.md) — <≤200-char hook with layer/fix-site + effort>. Surfaced <date>.`
...
- Don't bump versions, edit `CHANGELOG.md`, edit the locked `API.md`/`SPEC.md`,
  or touch code — this is backlog capture only.
