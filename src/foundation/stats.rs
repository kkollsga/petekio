...
//!
//! ## Family delegation & documented convention differences
//!
//! The unweighted linear (type-7) percentile is single-homed on the family
//! kernel `petektools::stats::percentile` — one definition of "the p-th
//! percentile" across the suite. Two reductions deliberately STAY LOCAL because
//! they are genuinely different algorithms, not duplicates of the petekTools
//! front-door:
//! - **`std` is the POPULATION standard deviation** (`n` denominator, weighted
//!   when constructed weighted). `petektools::stats::std_dev` is the UNBIASED
//!   SAMPLE deviation (`n − 1`). petekIO reduces whole populations (every
//!   defined node / sample of a surface or log), so the population moment is the
//!   correct one — not a bug, a convention.
//! - **weighted percentiles use the CUMULATIVE-WEIGHT (nearest-rank) rule**
//!   (`percentile_weighted`), the natural definition for thickness/PV-weighted
//!   subsurface aggregates. `petektools::stats::weighted_percentile` is a
//!   different (interpolating) weighted quantile; the two do not agree and are
...
impl Stats {
    ...
    /// Geometric mean of the **positive**, non-NaN values — `exp(mean(ln v))`.
    /// Non-positive and NaN values are skipped; `NaN` if none remain. The natural
    /// average for multiplicative / log-normal quantities such as permeability.
    ...
    fn from_sorted(sorted: Vec<f64>, weights: Vec<f64>) -> Stats {
        ...
        // std is the POPULATION deviation (n denominator) — local by convention
        // (see the module header), distinct from petektools' sample std_dev.
...
/// Linear-interpolation percentile (numpy "linear" / type-7) on ascending
/// `sorted`, `p ∈ [0, 1]`. Single-homed on the family kernel
/// `petektools::stats::percentile` (same type-7 / Excel-`PERCENTILE`
/// definition), which takes `p ∈ [0, 100]` and re-sorts internally (idempotent
/// on the already-ascending input). Empty input → `NaN`.
...
/// matching `weights`. LOCAL BY CONVENTION (see the module header): the
/// nearest-rank rule for thickness/PV-weighted subsurface aggregates, distinct
/// from `petektools::stats::weighted_percentile`'s interpolating definition —
/// the two do not agree and are not interchangeable.
