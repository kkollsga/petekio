...
```python
geo.set_element_tags("15/9-A1", ["field-a"])          # tag an element
geo.save("field.pproj")

petekio.GeoData.export("field.pproj", "share.pproj", ["field-a"])  # tagged subset → one file
...
```python
geo.put_model_section("model/field-a/props", ["field-a"], 1, payload_bytes)
version, data = geo.model_section("model/field-a/props")
