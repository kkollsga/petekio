...
fn summary_net_pay_and_averages_hand_calc() {
    ...
    // Voronoi thickness [5,10,10,10,5]; net pay = 5+10+5 = 20 ft = 6.096 m
    // (project unit is Feet; the DTO delivers metres, factor 0.3048).
    assert!(
        (s.net_pay_m.value - 6.096).abs() < 1e-9,
        "net_pay={}",
        s.net_pay_m.value
    );
    assert_eq!(s.net_pay_m.provenance, Provenance::Interpolated);
    ...
    assert!(s.owc_depth_m.is_none() && s.goc_depth_m.is_none());
}
...
fn spatial_horizons_and_boundary() {
    ...
    assert!(mi.summary.area_m2.value > 0.0);
    assert_eq!(mi.summary.area_m2.provenance, Provenance::Interpolated);
}
...
fn well_curves_are_canonical_and_harmonised() {
    ...
    // Positioned to world (x, y, z = negative-down elevation, matching Surface
    // z): well head (1200,1500), kb 0, vertical → first sample at md 2400 →
    // (1200, 1500, -2400).
