...
**IO — depend on `giga-segy`, do not green-room SEG-Y.**
...
rev2, active, MIT/Apache, `-core`/`-in`/`-out` split that already matches our
...
- **Phase 3 (3-D volumes):** SEG-Y (seismic & inversion cubes) via **`giga-segy`**
  (depend, don't green-room) behind a `SegyReader` trait; brick-tiled storage, not
